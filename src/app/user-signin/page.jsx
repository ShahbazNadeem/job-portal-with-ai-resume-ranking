...
export default function UserProfilePage() {
  ...
  useEffect(() => {
    if (!session?.user?.email) return; // no email, do nothing
    if (userData) return; // if there is already user then skip fetch
    ...
    fetch(`/api/user?email=${session.user.email}`)
      ...
      .then(data => {
        if (data.success) {
          ...
          updateUser(data.user); // store DB user in context
        }
        setLoading(false);
      })
